...
class MY_PostWidget extends WP_Widget {
  ...
  /** @see WP_Widget::widget */
                  ...
                  <?php if ( $title )
						...
						<?php $wp_query = new WP_Query( $args ); ?>
								...
								<li>
                  					...
                  					<time datetime="<?php the_time('Y-m-d\TH:i'); ?>"><?php the_time('d'); echo ' of '; the_time('F, Y'); ?></time>
                                    
									<?php if($excerpt_count!="") { ?>
                                        <div class="excerpt">
                                            <a href="<?php the_permalink() ?>"><?php $excerpt = get_the_excerpt(); echo my_string_limit_words($excerpt,$excerpt_count);?></a>
                                        </div>
									<?php } ?>
    ...
    function form($instance) {				
      ...
      <p><label for="<?php echo $this->get_field_id('category'); ?>"><?php _e('Category Slug:', 'e_repair'); ?> <input class="widefat" id="<?php echo $this->get_field_id('category'); ?>" name="<?php echo $this->get_field_name('category'); ?>" type="text" value="<?php echo $category; ?>" /></label></p>
			
			<p><label for="<?php echo $this->get_field_id('post_format'); ?>"><?php _e('Post format:', 'e_repair'); ?><br />
      ...
      <p><label for="<?php echo $this->get_field_id('sort_by'); ?>"><?php _e('Post order:', 'e_repair'); ?><br />
      ...
      <p><label for="<?php echo $this->get_field_id('count'); ?>"><?php _e('Posts per page:'); ?><input class="widefat" style="width:30px; display:block; text-align:center" id="<?php echo $this->get_field_id('count'); ?>" name="<?php echo $this->get_field_name('count'); ?>" type="text" value="<?php echo $count; ?>" /></label></p>
			...
			<p><label for="<?php echo $this->get_field_id('excerpt_count'); ?>"><?php _e('Excerpt length (words):'); ?><input class="widefat" style="width:30px; display:block; text-align:center" id="<?php echo $this->get_field_id('excerpt_count'); ?>" name="<?php echo $this->get_field_name('excerpt_count'); ?>" type="text" value="<?php echo $excerpt_count; ?>" /></label></p>
			 ...
			 <p><label for="<?php echo $this->get_field_id('linktext'); ?>"><?php _e('Link Text:', 'e_repair'); ?> <input class="widefat" id="<?php echo $this->get_field_id('linktext'); ?>" name="<?php echo $this->get_field_name('linktext'); ?>" type="text" value="<?php echo $linktext; ?>" /></label></p>
			 
			 <p><label for="<?php echo $this->get_field_id('linkurl'); ?>"><?php _e('Link Url:', 'e_repair'); ?> <input class="widefat" id="<?php echo $this->get_field_id('linkurl'); ?>" name="<?php echo $this->get_field_name('linkurl'); ?>" type="text" value="<?php echo $linkurl; ?>" /></label></p>
        <?php 
    }
