...
class MY_CommentWidget extends WP_Widget_Recent_Comments {
	...
	function MY_CommentWidget() {
		$widget_ops = array('classname' => 'widget_my_recent_comments', 'description' => __('My - Recent Comments','e_repair') );
		$this->WP_Widget('my-recent-comments', __('My - Recent Comments','e_repair'), $widget_ops);
	}
	...
	function widget( $args, $instance ) {
		...
		$title = apply_filters('widget_title', empty($instance['title']) ? __('My Recent Comments','e_repair') : $instance['title']);
...
?>
      ...
      <li class="recentcomments">
        ...
        <div class="comment-body">
          <a href="<?php echo get_comment_link( $comment->comment_ID ); ?>" title="<?php _e('Go to this comment', 'e_repair'); ?>"><?php echo strip_tags(substr(apply_filters('get_comment_text', $comment->comment_content), 0, $comment_len)); if (strlen($comment->comment_content) > $comment_len) echo '...';?></a>
        </div>
			</li>
            <?php
			endforeach; endif;?></ul>
